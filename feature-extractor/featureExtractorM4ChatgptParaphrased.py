...
if __name__ == "__main__":
    ...
    df = pd.read_json('./arxiv_chatGPT_paraphrased_100.jsonl', lines=True)
    characters = [")", "-", ";", ":", "?", "'"]

    for df in df.itertuples():
        num_sentence_human = count_sentences_per_paragraph(df.human_text)
        num_sentence_machine = count_sentences_per_paragraph(df.machine_text)

        num_words_human = count_words_per_paragraph(df.human_text)
        num_words_machine = count_words_per_paragraph(df.machine_text)

        character0_human = check_character_presence(df.human_text, characters[0])
        character1_human = check_character_presence(df.human_text, characters[1])
        character2_human = check_character_presence(df.human_text, characters[2])
        character3_human = check_character_presence(df.human_text, characters[3])
        character4_human = check_character_presence(df.human_text, characters[4])
        character5_human = check_character_presence(df.human_text, characters[5])
        ...
        character0_machine = check_character_presence(df.machine_text, characters[0])
        character1_machine = check_character_presence(df.machine_text, characters[1])
        character2_machine = check_character_presence(df.machine_text, characters[2])
        character3_machine = check_character_presence(df.machine_text, characters[3])
        character4_machine = check_character_presence(df.machine_text, characters[4])
        character5_machine = check_character_presence(df.machine_text, characters[5])
        ...
        std_dev_human = paragraph_sentence_length_std_dev(df.human_text)
        std_dev_machine = paragraph_sentence_length_std_dev(df.machine_text)

        sent_len_diff_human = max_length_difference_paragraph(df.human_text)
        sent_len_diff_machine = max_length_difference_paragraph(df.machine_text)

        count_short_sentences_in_paragraphs_human = count_short_sentences_in_paragraphs(df.human_text)
        count_short_sentences_in_paragraphs_machine = count_short_sentences_in_paragraphs(df.machine_text)

        count_long_sentences_in_paragraphs_human = count_long_sentences_in_paragraphs(df.human_text)
        count_long_sentences_in_paragraphs_machine = count_long_sentences_in_paragraphs(df.machine_text)


        words = ["although", "However", "but", "because", "this", "others", "researchers"]

        check_word0_human = check_words_in_paragraphs(df.human_text, words[0])
        check_word0_machine = check_words_in_paragraphs(df.machine_text, words[0])

        check_word1_human = check_words_in_paragraphs(df.human_text, words[1])
        check_word1_machine = check_words_in_paragraphs(df.machine_text, words[1])

        check_word2_human = check_words_in_paragraphs(df.human_text, words[2])
        check_word2_machine = check_words_in_paragraphs(df.machine_text, words[2])

        check_word3_human = check_words_in_paragraphs(df.human_text, words[3])
        check_word3_machine = check_words_in_paragraphs(df.machine_text, words[3])

        check_word4_human = check_words_in_paragraphs(df.human_text, words[4])
        check_word4_machine = check_words_in_paragraphs(df.machine_text, words[4])

        check_word5_human = check_words_in_paragraphs(df.human_text, words[5])
        check_word5_machine = check_words_in_paragraphs(df.machine_text, words[5])

        check_word6_human = check_words_in_paragraphs(df.human_text, words[6])
        check_word6_machine = check_words_in_paragraphs(df.machine_text, words[6])
        ...
        check_num_human = check_numbers_in_paragraphs(df.human_text)
        check_num_machine = check_numbers_in_paragraphs(df.machine_text)

        check_capitals_human = check_capitals_to_periods_ratio(df.human_text)
        check_capitals_machine = check_capitals_to_periods_ratio(df.machine_text)

        check_et_human = check_et_in_paragraphs(df.human_text)
        check_et_machine = check_et_in_paragraphs(df.machine_text)
        ...
        df1.to_csv('./features_chatgpt_paraphrased_100.csv', mode='a', index=False, header=header)
        header = False
    
    df = pd.read_csv('./features_chatgpt_paraphrased_100.csv')
    ...
    df.to_csv('./features_chatgpt_normalized_paraphrased_100.csv', index=False)
